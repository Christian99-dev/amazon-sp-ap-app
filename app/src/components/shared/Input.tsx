const Input = ({
  ...
  const getColorClasses = (status: "error" | "success" | "idle") => {
    switch (status) {
      case "error":
        return "bg-red-50 border-[2px] border-red-400 text-red-900  placeholder-red-700 ";
      case "success":
        return "bg-green-50 border-[2px] border-green-500 text-green-900  placeholder-green-700 ";
      case "idle":
    ...
    <div className="flex h-min">
      <input
        ...
        className={`${getColorClasses(
          ...
          !hasDeleteButton && "rounded-r-lg"
        } text-xs w-28 px-2 py-1.5 outline-none font-[600] placeholder:font-normal`}
        placeholder={placeholder}
      />
      {hasDeleteButton && (
        <span
          onClick={onDeleteButton}
          className="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-l-0 border-gray-300 cursor-pointer rounded-l-0 rounded-tr-md rounded-br-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600"
        >
          <svg
